 ...
 */
public class SupervisiorStaff_ReportAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public SupervisiorStaff_ReportAdapter(ArrayList<mpm_model> list, Context context, SupervisorStaff_Reports supervisorStaff_reports) {


        this.context = context;
        this.recyclerViewclick = (RecyclerViewclick) supervisorStaff_reports;
        this.list = list;
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder viewHolder, final int position) {
        ...
        if (viewHolder instanceof ReportViewHolder) {
            if (position < list.size()) {
                ...
                ((SupervisiorStaff_ReportAdapter.ReportViewHolder) viewHolder).mobileNumber.setText(data.getAttribute1());
                ((SupervisiorStaff_ReportAdapter.ReportViewHolder) viewHolder).remark.setText(data.getAttribute2());
                ((SupervisiorStaff_ReportAdapter.ReportViewHolder) viewHolder).date.setText(data.getArcDate().substring(0, data.getArcDate().length() - 3));
                ((SupervisiorStaff_ReportAdapter.ReportViewHolder) viewHolder).mobileNumber.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        recyclerViewclick.onclickList(position);
                    }
                });
                ((SupervisiorStaff_ReportAdapter.ReportViewHolder) viewHolder).remark.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        recyclerViewclick.onclickList(position);
                    }
                });
                ((SupervisiorStaff_ReportAdapter.ReportViewHolder) viewHolder).date.setOnClickListener(new View.OnClickListener() {
                    @Override
                    public void onClick(View view) {
                        recyclerViewclick.onclickList(position);
                    }
